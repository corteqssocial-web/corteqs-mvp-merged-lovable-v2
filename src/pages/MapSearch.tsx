...
import { MapPin, Search, Building2, Users, ChevronDown, Store, Navigation, Tag, Coffee, Gift, Ticket, Globe } from "lucide-react";
...
const cities: CityCoord[] = [
  { key: "dubai", label: "Dubai", country: "BAE", lat: 25.2, lng: 55.27 },
  { key: "abudhabi", label: "Abu Dhabi", country: "BAE", lat: 24.45, lng: 54.38 },
  { key: "berlin", label: "Berlin", country: "Almanya", lat: 52.52, lng: 13.41 },
  { key: "munich", label: "Münih", country: "Almanya", lat: 48.14, lng: 11.58 },
  { key: "frankfurt", label: "Frankfurt", country: "Almanya", lat: 50.11, lng: 8.68 },
  { key: "hamburg", label: "Hamburg", country: "Almanya", lat: 53.55, lng: 9.99 },
  { key: "washington", label: "Washington", country: "ABD", lat: 38.9, lng: -77.04 },
  { key: "newyork", label: "New York", country: "ABD", lat: 40.71, lng: -74.0 },
  { key: "losangeles", label: "Los Angeles", country: "ABD", lat: 34.05, lng: -118.24 },
  { key: "london", label: "Londra", country: "İngiltere", lat: 51.51, lng: -0.13 },
  { key: "manchester", label: "Manchester", country: "İngiltere", lat: 53.48, lng: -2.24 },
  { key: "amsterdam", label: "Amsterdam", country: "Hollanda", lat: 52.37, lng: 4.9 },
  { key: "rotterdam", label: "Rotterdam", country: "Hollanda", lat: 51.92, lng: 4.48 },
  { key: "paris", label: "Paris", country: "Fransa", lat: 48.86, lng: 2.35 },
  { key: "lyon", label: "Lyon", country: "Fransa", lat: 45.76, lng: 4.84 },
  { key: "vienna", label: "Viyana", country: "Avusturya", lat: 48.21, lng: 16.37 },
  { key: "zurich", label: "Zürih", country: "İsviçre", lat: 47.38, lng: 8.54 },
  { key: "toronto", label: "Toronto", country: "Kanada", lat: 43.65, lng: -79.38 },
  { key: "montreal", label: "Montreal", country: "Kanada", lat: 45.5, lng: -73.57 },
  { key: "sydney", label: "Sydney", country: "Avustralya", lat: -33.87, lng: 151.21 },
  { key: "doha", label: "Doha", country: "Katar", lat: 25.29, lng: 51.53 },
  { key: "madrid", label: "Madrid", country: "İspanya", lat: 40.42, lng: -3.7 },
  { key: "rome", label: "Roma", country: "İtalya", lat: 41.9, lng: 12.5 },
  { key: "stockholm", label: "Stockholm", country: "İsveç", lat: 59.33, lng: 18.07 },
  { key: "brussels", label: "Brüksel", country: "Belçika", lat: 50.85, lng: 4.35 },
  { key: "lisbon", label: "Lisbon", country: "Portekiz", lat: 38.72, lng: -9.14 },
];

const countryList = Array.from(new Set(cities.map(c => c.country))).sort();
...
const MapSearch = () => {
  ...
  // Use global country from navbar; fallback to BAE when "all"/empty
  const effectiveCountry = globalCountry && globalCountry !== "all" ? globalCountry : "BAE";
  ...
  const countryCityList = useMemo(
    () => cities.filter(c => c.country === effectiveCountry).sort((a, b) => a.label.localeCompare(b.label, "tr")),
    [effectiveCountry]
  );
  ...
  // Aggregate entities for active cities
  const entities = useMemo(() => {
    return activeCities.flatMap(c => mockEntities[c.key] || []);
  }, [activeCities]);

  const filtered = entityType === "all" ? entities : entities.filter(e => e.type === entityType);

  // Map center: use first active city as anchor (or selected city)
  const cityData = activeCities[0] || cities[0];
  ...
  const cityDisplayLabel = selectedCity === "all"
    ? `Tüm Şehirler - ${effectiveCountry}`
    : countryCityList.find(c => c.key === selectedCity)?.label || "Şehir Seçin";
